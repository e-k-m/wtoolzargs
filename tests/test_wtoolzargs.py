...
def func_filter(source):
    res = wtoolzargs.filter_(Hacker, source)
    return str(res.compile(compile_kwargs={"literal_binds": True}))


def func_order(source):
    res = wtoolzargs.order(Hacker, source)
    return str(
        [str(e.compile(compile_kwargs={"literal_binds": True})) for e in res]
    )


class TestWtoolzArgs(unittest.TestCase):
    def test_filter_(self):
        cases = [
            ...
            Case("a eq 10 and b eq 10", "hacker.a = 10.0 AND hacker.b = 10.0"),
            Case("a eq 10 or b eq 10", "hacker.a = 10.0 OR hacker.b = 10.0"),
            Case(
                "(a eq 10 or b eq 10) and c eq 10",
                "(hacker.a = 10.0 OR hacker.b = 10.0) AND hacker.c = 10.0",
            ),
            Case("a eq 'a'", "hacker.a = 'a'"),
            Case("a eq 'a' and b eq 'b'", "hacker.a = 'a' AND hacker.b = 'b'"),
